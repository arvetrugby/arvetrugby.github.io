...
function agregarBotonWhatsApp(jugador, mensaje) {
    ...
    btn.innerHTML = `
        ...
        <span>Notificá último Cambio</span>
    `;
...
function agregarBotonWhatsAppAdmin(telefonoAdmin, datosJugador) {
    ...
    btn.href = `https://wa.me/${String(telefonoAdmin).replace(/\D/g, '')}?text=${encodeURIComponent(
        `Hola, me acabo de registrar en ARVET.\n\n*Nombre:* ${datosJugador.nombre} ${datosJugador.apellido}\n*Email:* ${datosJugador.email}\n*DNI:* ${datosJugador.dni}\n\nPor favor aprueba mi registro para poder acceder al panel del equipo. ¡Gracias!`
    )}`;
    btn.target = '_blank';
    btn.className = 'btn-whatsapp latido-activo';
    btn.innerHTML = `
    ...
    msg.style.display = 'none';
        ...
        try {
            ...
            console.log('Respuesta:', result);
                ...
                // Si hay teléfono de admin y NO viene del admin, mostrar botón WhatsApp
                if (result.data.telefonoAdmin && !esAdmin) {
                    agregarBotonWhatsAppAdmin(result.data.telefonoAdmin, data);
